...
object WikipediaRanking {
   ...
   */
  private def langFilter(lang: String, text: String): Boolean = text.split(" ").contains(lang)

  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.filter(article => langFilter(lang, article.text)).count().toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(article =>
      langs.filter(lang => langFilter(lang, article.text))
